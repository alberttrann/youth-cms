...
| `api::project.project` | **Collection** | `true` | Dự án SDG: Tên, mô tả, chỉ số tác động, khu vực, `countriesCovered`, `partnerOrganization` (tổ chức đối tác co-host ngoài Y.O.U), `focusSdgs` (multi-enum), ảnh bìa, thư viện ảnh `gallery`, liên kết `member`. |
